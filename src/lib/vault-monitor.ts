...
import type { VaultHealth, VaultAllocation, BenchmarkComparison } from "../types.js";
...
export class VaultMonitor {
  ...
  private getVaultAddress(vault: string) {
    return vault === "earnUSD" ? EARN_USD_VAULT : EARN_ETH_VAULT;
  }

  async getVaultHealth(vaultName: string = "earnETH"): Promise<VaultHealth> {
    ...
    const allocations: VaultAllocation[] = protocols.map((p) => ({
      protocol: p.name,
  ...
  async getBenchmarkComparison(vaultName: string = "earnETH"): Promise<BenchmarkComparison> {
