import { z } from "zod";
export function registerVaultTools(server, vaultMonitor) {
    server.tool("vault_health", "Check Lido Earn vault health: total assets, current yield, protocol allocations, and benchmark comparison.", {
        vault: z
    ...
    server.tool("vault_alerts", "Get alerts and status changes for a Lido Earn vault. Returns actionable insights about yield, allocations, and risks.", {
        vault: z
            .enum(["earnETH", "earnUSD"])
            .default("earnETH")
    ...
    server.tool("vault_benchmark", "Compare Lido Earn vault yield against benchmarks: raw stETH APY and Aave v3 WETH supply rate.", {
        vault: z
            .enum(["earnETH", "earnUSD"])
            .default("earnETH")
