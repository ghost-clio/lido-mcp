import { z } from "zod";
export function registerStakingTools(server, lido) {
    server.tool("lido_stake", "Stake ETH to receive stETH via Lido. Returns ~1:1 stETH. stETH rebases daily to reflect staking rewards.", {
    ...
    server.tool("lido_unstake", "Request withdrawal from stETH back to ETH. Creates a withdrawal request NFT. Withdrawals take 1-5 days to finalize.", {
    ...
    server.tool("lido_wrap", "Wrap stETH into wstETH. wstETH is non-rebasing — its balance stays constant while its value accrues. Required for DeFi and L2 bridging.", {
    ...
    server.tool("lido_unwrap", "Unwrap wstETH back to stETH. You'll receive more stETH than you originally wrapped due to accrued rewards.", {
